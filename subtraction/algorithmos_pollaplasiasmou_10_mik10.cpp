...
//mpfr_sub_d(<metavliti oppou_tha_ekchorithi_to_apotelesma>,<metablhth_mpfr>,<stathera>,MPFR_RNDN)
//afairvw kai apo tiw metavlites typou f,d (gia diaforetiko precision).
...
if ((ii/100000)%2==1)
{
		mpfr_set_str(fs,"3.11888",10,MPFR_RNDN);
		mpfr_set_str(ds,"3.11888",10,MPFR_RNDN);
}
else
{

		mpfr_set_str(fs,"7.77777",10,MPFR_RNDN);
		mpfr_set_str(ds,"7.77777",10,MPFR_RNDN);
}
...
*/
			 if (mpfr_cmp_si(fz,10)>0)
							 {
								 mpfr_div_si(fz,fz,10,MPFR_RNDA);
								 mpfr_div_si(dz,dz,10,MPFR_RNDA);
							 }
		...
		else
		{
			...
			mpfr_div_si(fx,fx,10,MPFR_RNDN);
			mpfr_div_si(dx,dx,10,MPFR_RNDN);

//			mpfr_add_si(fz,fz,8,MPFR_RNDN);
//			mpfr_add_si(dz,dz,8,MPFR_RNDN);

			mpfr_mul(fz,fx,fs,MPFR_RNDN);
			mpfr_mul(dz,dx,ds,MPFR_RNDN);
			 if (mpfr_cmp_si(fz,10)>0)
				 {
					 mpfr_div_si(fz,fz,10,MPFR_RNDA);
					 mpfr_div_si(dz,dz,10,MPFR_RNDA);
