//Allazw WINDOW_LENGTH, SIZE_TMIMA, arxh_song kai myfile.open("....txt")
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <gmpxx.h>
...
{	
	...
	string string1;	
	int error_16_32_real, error_32_64_real, error_16_32_imag, error_32_64_imag;
...
//////////////////////////////////////////////////////////////////////
   ...
   //Anoigma arxeiou gia diavasma
	...
	myfile.open ("fft2_Spente_Le_Stelle_WL8M_arx500K.txt");
		...
		{ /* while the file still has numbers to be read */			
			...
			while(costas_read.good() && ff<2)
			{
				if(ff==0)
				{
					...
					//mpfr_printf("%.*Re\n",6,nums_read_16[deiktis_read].real.mpfr_ptr());

					//nums_read_32[deiktis_read].real=nums_read_16[deiktis_read].real;
					//mpfr_printf("%.*Re\n",6,nums_read_32[deiktis_read].real.mpfr_ptr());

					//nums_read_64[deiktis_read].real=nums_read_16[deiktis_read].real;
					//mpfr_printf("%.*Re\n",6,nums_read_64[deiktis_read].real.mpfr_ptr()); 
				}
				else
				{
					costas_read >>	nums_read_16[deiktis_read].imag;
					//mpfr_printf("%.*Re\n",6,nums_read_16[deiktis_read].imag.mpfr_ptr());

					//nums_read_32[deiktis_read].imag=nums_read_16[deiktis_read].imag;
					//mpfr_printf("%.*Re\n",6,nums_read_32[deiktis_read].imag.mpfr_ptr());

					//nums_read_64[deiktis_read].imag=nums_read_16[deiktis_read].imag;
					//mpfr_printf("%.*Re\n",6,nums_read_64[deiktis_read].imag.mpfr_ptr()); 
				}
				ff++;
			}	//while(costas_read.good() && ff<2)
