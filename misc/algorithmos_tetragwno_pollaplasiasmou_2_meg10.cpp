...
int main()
{
	...
	mpfr_t fz,fx,fs,fz_rand_tmp,fz_while_cond;
	mpfr_t dz,dx,ds,dz_rand_tmp;
	...
	mpfr_init2(fz_rand_tmp,bit_precision);
	mpfr_init2(fz_while_cond,bit_precision);

	mpfr_init2(dz,dl_bit_precision);
	mpfr_init2(dx,dl_bit_precision);
	mpfr_init2(ds,dl_bit_precision);
	mpfr_init2(dz_rand_tmp,dl_bit_precision);

	mpfr_init2(arithmos,bit_precision);
	mpfr_init2(d_arithmos,dl_bit_precision);


	mpfr_set_str(fx,"9.9666666",10,MPFR_RNDN);
	mpfr_set_str(dx,"9.9666666",10,MPFR_RNDN);
	...
	for(ii=0 ; ii<EPANAL ; ii++)
	{
		...
		mpfr_mul(fx,fz,fz,MPFR_RNDN);
		mpfr_mul(dx,dz,dz,MPFR_RNDN);


		if (mpfr_cmp_si(fx,10)>=0)
		{
			mpfr_div_si(fz,fx,10,MPFR_RNDN);
			mpfr_div_si(dz,dx,10,MPFR_RNDN);
		}
		else
		{
			fprintf(stderr,"Prosthesh!\n");

			srand(time(NULL));
			ostringstream convert;
			convert << rand();
			string temp_str = convert.str();
			arithmos=(char*) temp_str.c_str();
			mpfr_set_str(fz_rand_tmp,arithmos,10,MPFR_RNDN);
			mpfr_add_si(fz_rand_tmp,fz,fz_rand_tmp,MPFR_RNDN);
			mpfr_mul(fz_while_cond,fz_rand_tmp,fz_rand_tmp,MPFR_RNDN);
			while(mpfr_cmp_si(fz_while_cond,10)<0)	
			{		
				srand(time(NULL));
				ostringstream convert;
				convert << rand();
			...
			mpfr_set(fz,fz_rand_tmp,MPFR_RNDN);
			temp_str = convert.str();
			d_arithmos=(char*) temp_str.c_str();
			mpfr_set_str(dz_rand_tmp,d_arithmos,10,MPFR_RNDN);
			mpfr_add_si(dz,dz,dz_rand_tmp,MPFR_RNDN);


			//mpfr_add_si(fz,fz,6,MPFR_RNDN);
			//mpfr_add_si(dz,dz,6,MPFR_RNDN);
		...
		//if (ii%1000000==0)
		//{
			mpfr_fprintf(stderr,"error:%d ii:%ld \n",error,ii);
