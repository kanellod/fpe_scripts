...
int main()
{
	...
	long int ii,error,jj, synolikes_afaireseis, synolikoi_pollaplasiasmoi;
	...
	synolikes_afaireseis=0;
	synolikoi_pollaplasiasmoi=0;
...
*/
			...
			synolikoi_pollaplasiasmoi++;

		}
		else
		{
			synolikes_afaireseis++;
			...
			mpfr_set_str(arithmos,"3.500000",10,MPFR_RNDN);
			mpfr_set_str(d_arithmos,"3.5000000",10,MPFR_RNDN);
...
//									getchar();
		...
		if(error==16)
		{
			fprintf(stderr,"Anadromes: %ld\n",ii);
			fprintf(stderr,"Pollaplasiasmoi: %ld\n",synolikoi_pollaplasiasmoi);
			fprintf(stderr,"Afaireseis: %ld\n\n",synolikes_afaireseis);
		}
